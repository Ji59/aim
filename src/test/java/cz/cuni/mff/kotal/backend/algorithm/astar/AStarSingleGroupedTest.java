...
class AStarSingleGroupedTest {
	...
	void replanGroupTest() {
		final @NotNull SimulationGraph graph = new HexagonalGraph(3, 1, 1);

		final @NotNull List<Agent> agents = List.of(
		...
		final @NotNull Set<Triplet<Map<Agent, Pair<Integer, Set<Integer>>>, Map<Long, Map<Integer, Agent>>, Set<Agent>>> agentsGroups = new HashSet<>(6);
		@Nullable Triplet<Map<Agent, Pair<Integer, Set<Integer>>>, Map<Long, Map<Integer, Agent>>, Set<Agent>> group0CollisionTriplet = null;
		@Nullable Triplet<Map<Agent, Pair<Integer, Set<Integer>>>, Map<Long, Map<Integer, Agent>>, Set<Agent>> group1CollisionTriplet = null;
		final @NotNull Map<Long, Map<Integer, Set<Agent>>> invalidMovesMap = new HashMap<>();
		final @NotNull Map<Long, Map<Integer, Set<Agent>>> restGroupsConflictAvoidanceTable = new HashMap<>();

		for (@NotNull Agent agent : agents) {
			final @NotNull Map<Agent, Pair<Integer, Set<Integer>>> agentEntryExits = Map.of(agent, new Pair<>(agent.getEntry(), Set.of(agent.getExit())));
			final @NotNull Set<Agent> agentCollisionAgents = new HashSet<>();
			final @NotNull Map<Long, Map<Integer, Agent>> conflictAvoidanceTable = getConflictAvoidanceTable(agentEntryExits, 0);
			final @NotNull Triplet<Map<Agent, Pair<Integer, Set<Integer>>>, Map<Long, Map<Integer, Agent>>, Set<Agent>> collisionTriplet = new Triplet<>(agentEntryExits, conflictAvoidanceTable, agentCollisionAgents);
